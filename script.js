...
function generateProducts() {
    ...
    productsData.forEach(data => {
        ...
        let productCode1 = product.querySelector('.code1');
        let productLoc = product.querySelector('.loc');
        let productTags = product.querySelector('tags');

        productImg.src = data.imgSrc;
        productDescription.textContent = data.description;
        productCode.innerHTML = `DE<span>${data.code}</span>`
        productCode1.innerHTML = `PARA<span>${data.code}</span>`
        productLoc.innerHTML = `RUA:<span>${data.loc}</span>`
